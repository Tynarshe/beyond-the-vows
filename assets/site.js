...
if(form){document.querySelector('[name="_next"]').value=new URL('thank-you.html',window.location.href).href;form.addEventListener('submit',()=>{const button=form.querySelector('[type="submit"]');button.textContent='Sending your enquiry…';button.disabled=true;setTimeout(()=>{button.disabled=false;button.textContent='Send your enquiry ↗'},15000)});window.addEventListener('pageshow',()=>{const button=form.querySelector('[type="submit"]');button.disabled=false;button.textContent='Send your enquiry ↗'});}
